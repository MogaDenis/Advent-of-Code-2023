...
int get_hand_type(const hand& hand_to_check)
{
    ...
    {
        if (pair.second > maxOccurrences && pair.first != 'J')
        {
    ...
    std::map<char, int> cardsAfterJoker{};
    std::string cardsCopy;
    cardsCopy = hand_to_check.cards;
    for (char& card : cardsCopy)
    {
        if (card == 'J')
            card = maxCard;
    }

    for (const char& card : cardsCopy)
    {
        if (!cardsAfterJoker.contains(card))
            cardsAfterJoker[card] = 1;
        else
            cardsAfterJoker[card]++;
    }

    int uniqueCards = (int)cardsAfterJoker.size();
    ...
    maxOccurrences = 0;
    for (const auto& pair : cardsAfterJoker)
    {
...
std::map<char, int> cardsToValue{{'A', 14}, {'K', 13}, {'Q', 12}, {'T', 10}, {'9', 9},
